...
- [ ] T030 [US1] Create `infra/modules/compute/` Part B: `paperclip` Cloud Run service. Image = `paperclip_image_digest` from tfvars (digest, never tag). `containerPort: 3100` (Paperclip's default). `min-instances=2`, `max-instances` from tfvars, CPU=2, mem=2Gi. Service account = `paperclip-runtime-sa`. VPC connector = `paperclip-connector`. Plain env vars per `contracts/container-image.md`: `HOST=0.0.0.0`, `SERVE_UI=true`, `PAPERCLIP_HOME=/paperclip`, `PAPERCLIP_INSTANCE_ID=prod`, **`PAPERCLIP_DEPLOYMENT_MODE=authenticated`** (mode), **`PAPERCLIP_DEPLOYMENT_EXPOSURE=public`** (exposure — TWO separate vars per the source-code audit), `PAPERCLIP_PUBLIC_URL=https://${var.domain}`, **`PAPERCLIP_AUTH_DISABLE_SIGN_UP=true`** (invitation-only enforcement), `PAPERCLIP_SECRETS_STRICT_MODE=true`, `PAPERCLIP_STORAGE_PROVIDER=s3`, `PAPERCLIP_STORAGE_S3_BUCKET=paperclip-492823-uploads`, `PAPERCLIP_STORAGE_S3_ENDPOINT=https://storage.googleapis.com`, `PAPERCLIP_STORAGE_S3_REGION=us-central1`, `PAPERCLIP_STORAGE_S3_FORCE_PATH_STYLE=true`, plus the four Vertex env vars (`CLAUDE_CODE_USE_VERTEX=1`, `CLOUD_ML_REGION=global`, `ANTHROPIC_VERTEX_PROJECT_ID=paperclip-492823`, `ANTHROPIC_DEFAULT_SONNET_MODEL=claude-sonnet-4-6` — these flow through to the spawned `claude` subprocess). Secret-mounted env vars (4 from Phase 2 secrets module + 1 from database module): `PAPERCLIP_SECRETS_MASTER_KEY` (from `paperclip-master-key`), `BETTER_AUTH_SECRET` (from `paperclip-better-auth-secret`), `DATABASE_URL` (from `paperclip-database-url`), **`AWS_ACCESS_KEY_ID`** (from `paperclip-s3-access-key-id`), **`AWS_SECRET_ACCESS_KEY`** (from `paperclip-s3-secret-access-key`). The standard AWS SDK env var names are required because Paperclip's S3 provider creates an `S3Client` without explicit credentials and falls back to the AWS SDK credential chain. Label `service=paperclip`. `enable-execute-command=true`.

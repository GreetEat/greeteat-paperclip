...
- [X] T005 [P] Add `infra/docker/.hadolint.yaml` baseline rules
...
- [X] T016 [P] Create `infra/modules/secrets/`: data lookups for the three bootstrap-created secrets (paperclip-master-key, paperclip-s3-access-key-id, paperclip-s3-secret-access-key); conditional `google_secret_manager_secret_iam_member` bindings activated by passing `runtime_service_account_email` (Phase 2 passes null, Phase 3 passes the actual SA). The fourth secret (paperclip-database-url) is created+bound by the database module in Phase 3.
...
- [X] T019 Create `infra/docker/Dockerfile`: multi-stage Node 20 build. Build stage clones Paperclip at the pinned tag (`PAPERCLIP_VERSION` build arg), uses corepack-managed pnpm, runs install + build + prune --prod. Runtime stage installs minimal apt deps + pnpm, creates non-root `paperclip` user (uid 1001), copies the built tree, exposes port 8080 (Cloud Run injects PORT). **First-cut Dockerfile** — assumes Paperclip ships `pnpm build` and `pnpm start`; refine on first CI build if upstream uses different scripts.
- [X] T020 [P] Create `infra/docker/entrypoint.sh`: validates 13 required env vars, refuses to start if `PAPERCLIP_DEPLOYMENT_MODE != public` or `CLAUDE_CODE_USE_VERTEX != 1`, warns if `ANTHROPIC_API_KEY` is set (it shouldn't be), masks the DB password in the startup log, exec's `pnpm start` as PID 1 so it receives SIGTERM cleanly during Cloud Run rollouts.
- [X] T021 Create `.github/workflows/build-image.yml`: WIF auth via `google-github-actions/auth@v2` (provider path hardcoded to project number 233990667256), determines paperclip_version from workflow input or versions.tfvars, runs Hadolint via `hadolint/hadolint-action@v3.1.0`, builds the image with both `<version>` and `<sha>` tags, runs Trivy via `aquasecurity/trivy-action@0.28.0` (fails on HIGH/CRITICAL with no fix), pushes to Artifact Registry, extracts the immutable digest via `docker inspect`, posts a markdown summary with the digest for the operator to paste into versions.tfvars.
